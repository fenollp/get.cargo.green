...
record.py               records the hero terminal replay by running the build for real
...
assets/replay.json      recorded build output, generated by record.py — do not hand-edit
...
The hero terminal is not written here and is not a mock-up. It replays `assets/replay.json`, which
`./record.py` produces by really running

    CARGO_TARGET_DIR=$(mktemp -d) CARGOGREEN_RUNNER=none cargo green install --frozen cargo-nextest

and stamping every line of output with the second it appeared at. `app.js` schedules those lines
against a wall clock, so the pacing on screen is the pacing that was measured. Lines keep cargo's
own colours — bold green status verb, default foreground — rather than the page palette. The crate
count, the wall clock, and the plain-`cargo install` baseline in the caption all come from the same file:
re-run `./record.py` and the numbers update themselves. `caption:` in `## hero` supplies only the
...
repo_label: cargo green install --frozen cargo-nextest
caption: Real run, empty CARGO_TARGET_DIR
